    ...
    >
            ...
            <div className="p-8 flex-1 flex flex-col">
              <div className="mb-8">
                ...
                <div className="flex items-end gap-2 my-4">
                  <span className={`text-4xl font-black ${tier.highlight ? 'gradient-text-brand' : 'text-slate-900'}`}>
                    {tier.price}
                  </span>
                </div>
                <p className="text-xs text-slate-500 font-mono">{tier.period}</p>
                <p className="text-sm text-slate-600 mt-3 leading-relaxed">{tier.description}</p>
              </div>
                ...
                <ul className="space-y-3">
                  {tier.features.map((f) => (
                    <li key={f} className="flex items-start gap-2.5 text-sm text-slate-600">
                      <Check className={`w-4 h-4 mt-0.5 flex-shrink-0 ${tier.highlight ? 'text-brand-600' : 'text-green-600'}`} />
                      {f}
                    </li>
                  ))}
                </ul>
      ...
      <div className="mt-12 text-center">
        <p className="text-sm text-slate-600">
          🔒 <strong className="text-slate-700">Garantia de escopo:</strong> O que foi acordado é o que será entregue. Sem custos extras, sem surpresas.
        </p>
      </div>
    </Section>
  );
