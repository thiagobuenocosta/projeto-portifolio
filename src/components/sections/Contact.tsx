...
_Enviado via Rosa Bueno Studio Tech_`;
    ...
    return (
      <Section id="contato">
        <div className="max-w-2xl mx-auto text-center">
          <div className="glass-card p-12 relative overflow-hidden bg-white">
            <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-brand-500 to-cyan-500" />
            <CheckCircle className="w-16 h-16 text-green-600 mx-auto mb-6" />
            <h2 className="text-3xl font-black text-slate-900 mb-4">Mensagem Recebida! 🚀</h2>
            <p className="text-slate-600 mb-2">
              Obrigado, <strong className="text-slate-900">{form.name}</strong>!
            </p>
            <p className="text-slate-600 leading-relaxed">
              Nossa equipe analisou seus dados. Entraremos em contato em <strong className="text-slate-900">até 24h úteis</strong> com um diagnóstico inicial estratégico.
            </p>
            <Button 
              variant="secondary" 
              className="mt-8"
  ...
  return (
    <Section 
      id="contato"
      background={
        <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-[900px] h-[400px] bg-brand-600/5 rounded-full blur-[120px]" />
      }
    >
      <div className="grid lg:grid-cols-2 gap-16 items-start">
          ...
          <div className="space-y-4">
            {[
              ...
              <div key={item.text} className="flex items-center gap-3 text-sm text-slate-600">
                <span className="text-brand-650">{item.icon}</span>
                <span>{item.text}</span>
              </div>
            ))}
          </div>
        ...
        <div className="glass-card p-8 bg-white/90">
          ...
          >
            ...
            <div>
              <label htmlFor="problem" className="block text-xs font-semibold text-slate-600 mb-1.5">
              ...
              <textarea
                ...
                required
                className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm text-slate-800 placeholder:text-slate-400 focus:outline-none focus:border-brand-600 focus:ring-1 focus:ring-brand-600/30 transition-all resize-none"
              />
            </div>

            <div className="grid sm:grid-cols-2 gap-4">
              <div>
                <label htmlFor="cost" className="block text-xs font-semibold text-slate-600 mb-1.5">
                ...
                <select
                  ...
                  className="w-full bg-white border border-slate-200 rounded-xl px-4 py-3 text-sm text-slate-800 focus:outline-none focus:border-brand-600 transition-all"
                >
            ...
            <p className="text-[10px] text-slate-500 text-center uppercase tracking-widest font-mono pt-2">
      ...
      >
        {modalType === 'validation' ? (
          <div className="flex items-start gap-4">
            ...
            <p className="text-slate-800">{modalMessage.text}</p>
          </div>
        ) : (
          <div className="space-y-4">
            ...
            <button
              ...
              className="w-full p-4 rounded-2xl bg-gradient-to-br from-[#25D366]/10 to-transparent border border-[#25D366]/30 hover:border-[#25D366]/60 transition-all group text-left relative overflow-hidden"
            >
              <div className="flex items-center gap-4 relative z-10">
                ...
                <div>
                  <p className="text-slate-900 font-bold">WhatsApp (Prioridade)</p>
                  <p className="text-xs text-[#20ba5a] font-semibold uppercase tracking-wider">Resposta em minutos</p>
                </div>
              </div>
              <div className="mt-3 flex items-center gap-2 text-[10px] text-slate-500 uppercase tracking-widest font-mono">
                <Zap className="w-3 h-3 text-amber-500" /> Contato imediato com Tech Lead
              </div>
            </button>
            ...
            <button
              ...
              className="w-full p-4 rounded-2xl bg-slate-50 border border-slate-200 hover:border-brand-500/50 hover:bg-brand-50/20 transition-all group text-left"
            >
              <div className="flex items-center gap-4">
                <div className="w-12 h-12 rounded-xl bg-white border border-slate-200 flex items-center justify-center group-hover:border-brand-500 transition-colors">
                  <Mail className="w-6 h-6 text-slate-600 group-hover:text-brand-600" />
                </div>
                <div>
                  <p className="text-slate-900 font-bold group-hover:text-brand-650 transition-colors">E-mail (Formal)</p>
                  <p className="text-xs text-slate-500 font-medium uppercase tracking-wider">Diagnóstico em até 24h</p>
                </div>
              </div>
              <div className="mt-3 flex items-center gap-2 text-[10px] text-slate-500 uppercase tracking-widest font-mono">
                <ShieldCheck className="w-3 h-3 text-brand-600" /> Processo qualificado e detalhado
              </div>
            </button>
